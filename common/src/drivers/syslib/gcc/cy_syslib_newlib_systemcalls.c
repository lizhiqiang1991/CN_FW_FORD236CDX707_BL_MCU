...
__attribute__((weak, used)) void *_sbrk(int incr) {
...
__attribute__((weak, used)) int _isatty(int file) {
  return 1;
}
...
__attribute__((weak, used)) void _exit(int status) {
  __asm("BKPT #0");
}

__attribute__((weak, used)) void _kill(int pid, int sig) {
  return;
}
